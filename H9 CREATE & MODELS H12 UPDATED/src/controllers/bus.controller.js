...
const deleteBus = async (req, res) => {
  try {
    ...
    const bus =await busService.getBusList()
    let bus_id = bus.map((item)=> {return item.id})
    let data = bus_id.find((item) => {return item === id.toString()})
...
const updateBus = async (req, res) => {
  try {
    ...
    const cateExists = await busService.getBusById(busId);
    if (!cateExists) {
...
module.exports = { createBus, getBus, deleteBus, updateBus };
