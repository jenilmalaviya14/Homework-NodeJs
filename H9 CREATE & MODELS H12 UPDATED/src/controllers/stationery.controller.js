...
const updateStationery = async (req, res) => {
  try {
    const stationeryId = req.params.Id;

    const cateExists = await stationeryService.getStationeryById(stationeryId);
    if (!cateExists) {
    ...
    res.status(200).json({
      success: true,
      message: "Stationery details update successfully!",
    });
  } catch (error) {
    res.status(400).json({
      success: false,
...
module.exports = { createStationery, getStationery, deleteStationery, updateStationery };
