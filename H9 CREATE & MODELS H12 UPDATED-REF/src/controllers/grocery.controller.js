...
const deleteGrocery = async (req, res) => {
  try {
    ...
    let hotel_id = grocery.map((item)=> {return item.id})
...
const updateGrocery = async (req, res) => {
  try {
    ...
    const cateExists = await groceryService.getGroceryById(groceryId);
    if (!cateExists) {
...
module.exports = { createGrocery, getGrocery, deleteGrocery, updateGrocery };
